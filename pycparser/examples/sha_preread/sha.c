...
void sha_stream(SHA_INFO *sha_info, char *file_buffer, int flen)
{
    int i;
    ...
    */
    int j;
    // For each block,
    for (j = 0; j < flen; j+= BLOCK_SIZE) {
        // i is size of data block (< BLOCK_SIZE for last block)
        i = flen - j;
        if (i > BLOCK_SIZE) {
            i = BLOCK_SIZE;
        }
        // Copy from file_buffer into data buffer
        memcpy(data, file_buffer + j, i);
        // Run sha algorithm
        sha_update(sha_info, data, i);
    }
    sha_final(sha_info);
