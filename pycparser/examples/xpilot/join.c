...
static void Input_loop()
{
  ...
  int Client_input_result;
  Client_input_result = Client_input(2);
  if (Client_input_result == -1) {
    return;
  }
  if (Net_flush() == -1) {
    return;
  ...
  // dlo: variables for keeping track of time
  struct timeval start, end;
  // dlo: file for storing times
  FILE *time_fp;
  time_fp = fopen("times.txt", "w");
  if (time_fp == NULL) {
    fprintf(stderr, "Unable to open output file times.txt\n");
    exit(1);
  }
  ...
  for (tfds = rfds; ; rfds = tfds) {
    ...
    if (FD_ISSET(clientfd, &rfds)) {
        int Client_input_result;
        Client_input_result = Client_input(1);
      if (Client_input_result == -1) {
        return;
      }
      if (Net_flush() == -1) {
    ...
    // dlo: print out time of this frame
    fprintf(time_fp, "time 0 = %d us\n", (end.tv_sec - start.tv_sec)*1000000 + (end.tv_usec - start.tv_usec));
  }
}
#endif	/* _WINDOWS */
	 ...
	 char *nick, int my_team, char *display, unsigned version)
{
    ...
    Client_init_result = Client_init(server_name, version);
    if (Client_init_result == -1) {
	return -1;
    }
    ...
    IFWINDOWS( Progress("Client_setup"); )
      int Client_setup_result;
    Client_setup_result = Client_setup();
    if (Client_setup_result == -1) {
	Net_cleanup();
	Client_cleanup();
	return -1;
    }
    ...
    IFWINDOWS( Progress("Client_start"); )
      int Client_start_result;
    Client_start_result = Client_start();
    if (Client_start_result == -1) {
	errno = 0;
...
static void Free_selectionAndHistory()
{
    if (HistoryBlock) {
	free(HistoryBlock);
	HistoryBlock = NULL;
