...
class FuncDefRenameVisitor(c_ast.NodeVisitor):
  ...
  def visit_FuncDef(self, node):
    ...
    node.decl.type.type.declname += "_reduced"
...
class DataDependencyVisitor(c_ast.NodeVisitor):
  ...
  def slice_data(self, node):
    ...
    if self.specific_array_index:
      for i in self.id_visitor.IDs:
        # Check that ID does not match base name whenn slicing for specific
  ...
  def get_ArrayRef_name(self, node):
    # If slicing is set up for specific array index, then compare based on full
    # string of array reference. This is meant to only be used for slicing for
    # specific loop_counter[i]
    if self.specific_array_index:
...
if __name__ == "__main__":
  if len(sys.argv) > 2:
    ...
    print "usage: slice_array_index.py filename var0 [var1 ...]"
    sys.exit()
