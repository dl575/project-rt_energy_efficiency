...
if [[ $2 ]] ; then
    ...
    mv output_large.txt $PROJECT_PATH/dvfs_sim/data_odroid/$SAVED_FOLDER/$BENCHMARK/$2
    echo $MAX_FREQ > /sys/devices/system/cpu/$WHICH_CPU/cpufreq/scaling_max_freq 
    echo [ done ]
    exit 1
fi
...
do 
    ...
    echo $i"..."
    taskset $TASKSET_FLAG ./runme_large.sh
    mv output_large.txt $PROJECT_PATH/dvfs_sim/data_odroid/$SAVED_FOLDER/$BENCHMARK/$i
done
