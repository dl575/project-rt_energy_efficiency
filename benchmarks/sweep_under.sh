...
#      ./runAll.sh $i $1 offline.txt ${SWEEP[$j]}
      sed -i -e 's/'"UNDER_PENALTY (100)"'/'"UNDER_PENALTY (1)"'/g' $BENCH_PATH/$COMMON_FILE
      taskset 0xff ./buildAll.sh $i $1 set_prediction_online ${SWEEP[$j]}
      ./runAll.sh $i $1 online-1 ${SWEEP[$j]}
      sed -i -e 's/'"UNDER_PENALTY (1)"'/'"UNDER_PENALTY (10)"'/g' $BENCH_PATH/$COMMON_FILE
      taskset 0xff ./buildAll.sh $i $1 set_prediction_online ${SWEEP[$j]}
      ./runAll.sh $i $1 online-10 ${SWEEP[$j]}
      sed -i -e 's/'"UNDER_PENALTY (10)"'/'"UNDER_PENALTY (100)"'/g' $BENCH_PATH/$COMMON_FILE
      taskset 0xff ./buildAll.sh $i $1 set_prediction_online ${SWEEP[$j]}
      ./runAll.sh $i $1 online-100 ${SWEEP[$j]}
      sed -i -e 's/'"UNDER_PENALTY (100)"'/'"UNDER_PENALTY (1000)"'/g' $BENCH_PATH/$COMMON_FILE
      taskset 0xff ./buildAll.sh $i $1 set_prediction_online ${SWEEP[$j]}
      ./runAll.sh $i $1 online-1000 ${SWEEP[$j]}
